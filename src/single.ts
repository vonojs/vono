	...
	return [
		...
		clearOutdir(vono.adaptor.outputDirectory),
		{
				...
				return {
					...
					build: {
						...
						rollupOptions: ssr(
							...
							{
								input: [tools.resolveExt(vono.clientEntry), existsSync(p.join(root, 'index.html')) && "/index.html"].filter(
									String,
								) as string[],
								output: {
			...
			configureServer: (server) => {
				return async () => {
					if (!tools.resolveExt(p.join(viteConfig.root, vono.serverEntry))) {
						throw new Error(
					...
					server.middlewares.use(async (nodeRequest, nodeResponse, next) => {
						try {
							const request = tools.createRequest(nodeRequest);
							const response = await devHandler(request);
							return tools.handleNodeResponse(response, nodeResponse);
						} catch (e) {
							console.error(e);
							next();
						}
