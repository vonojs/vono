...
const isRpcPath = (path: string) =>
	path.endsWith(".rpc.ts") ||
	path.endsWith(".rpc.js") ||
	path.endsWith(".rpc.tsx") ||
	path.endsWith(".rpc.jsx");
	...
	const generateManifest = async (config: vite.ResolvedConfig) => {
		...
		for (const file of files) {
			if (file.isFile() && isRpcPath(file.name)) {
				result[String(Math.random()).substring(2)] = p.join(
					file.path,
					file.name,
				);
			}
