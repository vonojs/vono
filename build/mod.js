    ...
    vfsPlugin({ vfs: useVFS(), alias: "#vono" }),
    {
      ...
      config: (vite) => {
        ...
        return {
          ...
          build: {
            emptyOutDir: !ssr,
            outDir: ssr(true) ? join(root, vono2.adaptor.outputDirectory) : join(root, vono2.adaptor.outputDirectory, "client"),
            manifest: !ssr(true),
            ssrEmitAssets: false,
            ssr: ssr(true),
            rollupOptions: ssr({
      ...
      configResolved: async (vite) => {
        ...
        useVFS().add({
          path: "entry",
          serverContent: () => `export {default} from "${slash(join(viteConfig.root, vono2.serverEntry))}";`
        });
        let buildctx;
        if (viteConfig.build?.ssr) {
          buildctx = {
            ...await fs.readFile(join(vono2.adaptor.outputDirectory, "vono.json"), "utf-8").then((content) => JSON.parse(content))
          };
        } else {
          buildctx = {};
        }
        useVFS().add({
          path: "buildctx",
          serverContent: () => `export default ${JSON.stringify(buildctx, null, 2)}`
        });
      },
      configureServer: (server) => {
        return async () => {
          if (!resolveExt(join(viteConfig.root, vono2.serverEntry))) {
            throw new Error(`Could not find server entry @ ${vono2.serverEntry}. Please provide a path to a file that exports a default handler function with the signature: (request: Request) => Response | Promise<Response>`);
          }
          updateHandler = async () => {
            devHandler = (await server.ssrLoadModule(vono2.adaptor.developmentRuntime)).default;
          };
          await updateHandler();
      ...
      handleHotUpdate: async (ctx) => {
        const containsEntry = (mod) => {
          if (!mod.id)
            return false;
          if (stripExt(mod.id) === slash(join(viteConfig.root, vono2.serverEntry))) {
            return true;
          }
          for (const dep of mod.importers) {
            if (containsEntry(dep))
              return true;
          }
          return false;
        };
      ...
      writeBundle: {
        ...
        handler: async () => {
          if (!viteConfig.build?.ssr) {
            await fs.writeFile(join(vono2.adaptor.outputDirectory, "vono.json"), JSON.stringify({
